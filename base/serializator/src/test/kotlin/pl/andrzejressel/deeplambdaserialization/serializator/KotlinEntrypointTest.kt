...
class KotlinEntrypointTest {

  private val serializedOne = "1".encodeToByteArray()
  private val serializedTwo = "2".encodeToByteArray()
  private val serializedThree = "\"3\"".encodeToByteArray()

  @Test
  fun serializableFunctionShouldHave1Function() {
    val examples = BuildInfo.location.resolve("build/examples")
    val className = JavaClassName(examples.resolve("kotlin_serializablefunction.txt").readText())
  ...
  fun serializableInputFunctionShouldHave2Functions() {
    val examples = BuildInfo.location.resolve("build/examples")
    val className =
        JavaClassName(examples.resolve("kotlin_serializableinputfunction.txt").readText())
    val jar = examples.resolve("jars").resolve("${NameUtils.getJarName(className)}.jar")
  ...
  fun serializableInputOutputFunctionShouldHave4Functions() {
    val examples = BuildInfo.location.resolve("build/examples")
    val className =
        JavaClassName(examples.resolve("kotlin_serializableinputoutputfunction.txt").readText())
    val jar = examples.resolve("jars").resolve("${NameUtils.getJarName(className)}.jar")
